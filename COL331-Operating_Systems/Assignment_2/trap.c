...
trap(struct trapframe *tf)
{
  ...
  // if(myproc())
  //   cprintf("inside trap myproc()->pending_custom_signal = %d \n", myproc()->pending_custom_signal);
  ...
  {
    // cprintf("---------------------------pid = %d cpu_ticks = %d wait_ticks = %d\n ", myproc()->pid, myproc()->cpu_ticks, myproc()->wait_ticks);
    myproc()->cpu_ticks++;
    update_wait_ticks();
    // cprintf("pid = %d ", myproc()->cpu_ticks);
    yield();
  }
