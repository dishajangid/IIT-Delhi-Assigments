...
int main(int argc, char* argv[]) {
  ...
  while (std::getline(input_file, line)) {
    ...
    double es = stock_data.front().price;
    double el = stock_data.front().price;
    double macd = es - el;
    double s = 0;
    ...
    for (int i=1; i < stock_data.size(); ++i) {
            ...
            es = (1-as) * es + as * stock_data[i].price;
            el = (1-al) * el + al * stock_data[i].price;
            s = (1 - a) * s + a * macd;
            macd = es - el;
    ...
    double cashflow, k, l = 0.0;
    int i = 0;
    l = stock_data.back().price;

    while(i<signals.size()) {
        if(i == 0) cashflow = 0;
        else cashflow -= signals[i] * (stock_data[i].price );//- result_data[i].price);
        if(i == signals.size() - 1){
          k = cashflow;
        }
