...
int main(int argc, char* argv[]) {
    ...
    for (size_t i = z; i < stock_data.size(); ++i) {
          ...
          for (int j = 0; j < n; ++j) {
              gain += std::max(0.0, stock_data[i - j].price - stock_data[i - j - 1].price);

              loss += std::max(0.0, stock_data[i - j - 1].price - stock_data[i - j].price);
          ...
          rsi = 100 - 100/(1+rs);
            ...
            if (rsi < oversold_threshold && f<x) {
                signals.push_back(1); // Buy
                f+=1;
            } else if (rsi > overbought_threshold && f>-x) {
